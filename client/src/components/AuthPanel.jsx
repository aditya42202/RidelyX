  ...
  return <div className="auth-page">
    ...
    <section className="auth-form-side" aria-labelledby="auth-form-title">
      <div className={`auth-card ${isForgot ? 'forgot-card' : ''}`}>
        ...
        {isLogin && !otpStep && <div style={{ display: 'grid', gap: '8px', fontSize: '10px' }}><button type="button" className="otp-link" onClick={requestEmailOtp}><Mail size={15} aria-hidden="true" /> Use email OTP instead <span aria-hidden="true">→</span></button><button type="button" className="otp-link" onClick={() => { setMode('phone'); setError(''); setSuccessMessage('') }} style={{ borderColor: '#13b9a7', color: '#08a995' }}><MapPin size={15} aria-hidden="true" /> Sign in with mobile <span aria-hidden="true">→</span></button></div>}
